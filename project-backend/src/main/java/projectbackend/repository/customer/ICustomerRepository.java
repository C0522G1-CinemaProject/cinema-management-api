...
public interface ICustomerRepository extends JpaRepository<Customer, Integer> {
    ...
    @Query(value = "select customer.id as id, customer.name as name, count(ticket.id) as tickets," +
            ...
            " where show_times.date_projection < curdate() and ticket.status_ticket = 1 " +
    ...
    @Query(value = "select customer.id as id, customer.name as name, count(ticket.id) as tickets," +
            ...
            " join seat_type on seat_type.id = seat_room.seat_type_id " +
            " join saving_point on saving_point.customer_id= customer.id " +
            " where show_times.date_projection < curdate() and ticket.status_ticket = 3 and show_times.date_projection >=  DATE_SUB(curdate(),INTERVAL :numberMonth MONTH)  " +
            "group by customer.id " +
            "order by sum(seat_type.price) " +
            "limit 100",
            nativeQuery = true)
